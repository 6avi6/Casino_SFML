...
void BoardsForBets::initBoard()
{
    ...
    std::shared_ptr<SelectedButton> button = std::make_shared<SelectedButton>(
        ...
        sf::Color::Green, "0", font, 0
    );
    boardFields.push_back(button);

    // Define the standard European roulette sequence and colors
    const int rouletteNumbers[36] = {
    ...
    const sf::Color colors[36] = {
        sf::Color::Black, sf::Color::Red, sf::Color::Black, sf::Color::Red, sf::Color::Black, sf::Color::Red,
        sf::Color::Black, sf::Color::Red, sf::Color::Black, sf::Color::Red, sf::Color::Red, sf::Color::Black,
        sf::Color::Red, sf::Color::Black, sf::Color::Red, sf::Color::Black, sf::Color::Red, sf::Color::Black,
        sf::Color::Black, sf::Color::Red, sf::Color::Black, sf::Color::Red, sf::Color::Black, sf::Color::Red,
        sf::Color::Black, sf::Color::Red, sf::Color::Black, sf::Color::Red, sf::Color::Red, sf::Color::Black,
        sf::Color::Red, sf::Color::Black, sf::Color::Red, sf::Color::Black, sf::Color::Red, sf::Color::Black
    };
    ...
    for (int j = 1; j < cols + 1; ++j) {
        for (int i = 0; i < rows; ++i) {
            ...
            auto button = std::make_shared<SelectedButton>(buttonPosition, sizeOfOneField, color, numStr, font, number);
            boardFields.push_back(button);
    ...
    // Sort boardFields based on their values
    std::sort(boardFields.begin(), boardFields.end(), [](std::shared_ptr<SelectedButton> a, std::shared_ptr<SelectedButton> b) {
...
float BoardsForBets::calculateWinnings(int resultNumber) const {
    ...
    if (resultNumber < 0 || resultNumber >= 37) {
        std::cerr << "Error: resultNumber out of range" << std::endl;
        return multiplier;
    }
    ...
    if (boardFields.size() < 2) {
        std::cerr << "Error: boardFields size is less than 2" << std::endl;
        return multiplier;
    }
    ...
    if (multiplier == 0.0f) {
        for (int i = 0; i < boardFields.size(); i++) {
            ...
            if (boardFields[i]->getSelected() && boardFields[i]->getValue() >= 0) {

                if (boardFields[i]->getValue() == resultNumber) {
